...
def train(model, training_data, validation_data, optimizer, scheduler, device, opt, writer):
    ...
    for epoch_i in range(opt.epoch):
        ...
        writer.add_scalar('acc/train', 100*train_accu, epoch_i)
        writer.add_scalar('loss/train',   train_loss, epoch_i)

        writer.add_scalar('acc/val', 100*valid_accu, epoch_i)
        writer.add_scalar('loss/val',   valid_loss, epoch_i)
...
def main():
    ...
    parser.add_argument('-optim', type=str, choices=['adam', 'sgd', 'nero', 'lamb', 'adamp', 'adamod'])
    ...
    if opt.optim == 'adam':
        ...
        optimizer = AdamP(transformer.parameters(), lr=opt.lr, betas=(0.9, 0.999), weight_decay=1e-2)
    elif opt.optim == 'adamod':
        optimizer = AdaMod(transformer.parameters(), lr=opt.lr, beta3=0.999)
